...
| 説明変数の重なり（発展） | `Pclass` と `Fare` などの関係に注意する |
...
| 10-18分 | Colab起動、Drive準備 | 作業環境を確認 |
| 18-30分 | データ読み込み | `df.head()` まで |
| 30-43分 | 欠損値と列の意味の確認 | 処理方針をメモ |
| 43-61分 | グループ別生存率の可視化 | 傾向を説明 |
| 61-75分 | 最小限の前処理 | モデルに入る形にする |
| 75-90分 | 分類モデルの評価 | 正解率・混同行列 |
| 90-100分 | Bank Marketingとデータ探索課題 | 授業後課題の方針を決める |
...
当日はTitanic例題を最後まで実行する。Bank Marketingの実行と公開データ候補メモは、授業後にClassroomの締切までに完成させる。
...
<div class="callout">
授業中はTitanic例題の実行と確認を優先する。Bank Marketingと公開データ候補は、同じノートブックの後半に追記する。
</div>
...
3. AI使用記録
4. 振り返りコメント
5. Bank Marketingの分析方針
6. 自分で探したデータ候補の出典URLと選定理由

<div class="callout small">
AI使用記録には、「AIはAと答えたが、実行結果・表・グラフを確認するとBだったので、Cに修正した」という形で、検証と修正の過程を残す。
</div>
...
## 研究でのAI活用例：SafeComp 2026

著者のSafeComp 2026向け研究では、自動車安全規格の採用状況に関するアンケートデータの整理・集計・可視化にAIを用いた。

| AIが役立ったこと | 人間が確認したこと |
|---|---|
| 集計コードの下書き | 変数の定義と回答数 |
| グラフ案の作成 | 軸、凡例、単位、外れ値 |
| 統計処理の候補提示 | 検定方法、効果量、解釈 |
| レポート文の下書き | 研究文脈との整合性 |

<div class="callout">
AIは分析の下書きには有効だが、数値・統計処理・研究としての解釈は人間が検証する。
</div>
...
|---|---|
| セル | コードや説明文を書く単位 |
| Shift + Enter | セルを実行して次へ進む |
| `[*]` | 実行中 |
| エラー表示 | まず最後の数行を読む |
| ランタイムの再起動 | 変数が消えるので、上から再実行する |

<div class="callout small">
分からないエラーが出たときは、エラー文、直前に実行したセル、期待していた結果をAIに渡す。
</div>
...
Driveに置いた `train.csv` を読み込む。見つからない場合はアップロードに切り替える。

```python
from pathlib import Path
from google.colab import files

data_path = Path("/content/drive/MyDrive/information_theory2026/train.csv")

if not data_path.exists():
    print("train.csv をアップロードしてください")
    files.upload()
...
2. 前処理で迷ったこと
3. AIが役に立った場面
4. AIは何と言い、何を確認し、どう直したか
5. Bank Marketingで試す分析方針
6. 次に分析したい公開データの候補
...
- 結果の分析をテキストセルに書いた
- AIの回答を、実行結果・数値・グラフで確認した記録を書いた
- Bank Marketingで試す分析方針を書いた
...
<div class="callout">
提出ノートブックでは、コードの実行結果だけでなく、判断理由、AI出力を確かめて修正した過程、Bank Marketingの方針、次に分析したいデータ候補を書く。
</div>
...
| 混同行列 | `[[98, 12], [23, 46]]` |
| 生存者再現率 | 0.667 |
| 係数例 | `Sex` +2.560 / `Pclass` -1.092 / `Fare` +0.002 |

<div class="callout small">
配布した `train.csv` での参考値。ライブラリのバージョンや丸め方により、小数点以下が少し変わることがある。
</div>
