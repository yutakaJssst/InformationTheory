...
各テストケースにはコメントで何をテストしているか説明してください。
...
<div class="prompt-box">
以下のPythonコードをレビューし、以下の観点で問題を指摘してください。
1. バグや潜在的なエラー
2. パフォーマンスの問題
3. 可読性の問題
4. セキュリティの問題

各指摘には重要度（高/中/低）と修正案を含めてください。
...
<div class="prompt-box">
以下のPythonコードに対して、以下を生成してください。
1. 各関数のdocstring（Google style）
2. モジュール全体のREADME（使い方、依存関係、実行方法）
3. 使用例（example code）
...
- 共同研究者との <span class="blue">**情報共有**</span>
- 将来の自分のための <span class="orange">**記録**</span>
...
<div class="prompt-box">
以下のコードをレビューしてください。
観点: 可読性、効率性、エラー処理、セキュリティ
各指摘に重要度（高/中/低）と修正案をつけてください。
</div>
