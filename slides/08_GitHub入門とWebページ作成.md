...
「大学院生の自己紹介Webページを作成してください。HTML/CSSで、以下のセクションを含めてください:<br>
- 名前と写真エリア<br>
- 研究テーマ<br>
- 所属（日本大学大学院理工学研究科）<br>
- 連絡先<br>
- 研究業績<br><br>
モダンでシンプルなレスポンシブデザインにしてください。1つのHTMLファイルにCSSも含めてください」
</div>

### 使用ツール
- <span class="teal">**Gemini Pro**</span>（推奨・大学で無料利用可）/ ChatGPT / Claude
...
1. **クローン**: GitHub Desktopで作成したリポジトリをクローン
   - File → Clone Repository → 作成したリポジトリを選択
2. **ファイル配置**: クローンしたフォルダに `index.html` をコピー
3. **変更確認**: GitHub Desktopに変更が表示される
4. **コミット**:
   - Summary に「自己紹介ページを追加」と入力
...
2. <span class="teal">**GitHub Desktopで確認**</span>: 変更差分が自動表示される
3. <span class="teal">**コミット**</span>: 変更内容を説明するメッセージを入力してコミット
4. <span class="teal">**プッシュ**</span>: 「Push origin」でGitHubに反映
5. <span class="teal">**確認**</span>: ブラウザで `https://ユーザー名.github.io/` を再読み込み
...
| 10分 | Gemini Proで自己紹介WebページのHTMLを生成 |
