...
- [ ] **学振DC1申請書の演習を発展課題として追加検討**（2026-04-29 メモ）
  - **動機**: 履修者はM1中心で博士進学希望者の比率もそれなり。ガクチカ（過去経験の言語化）と対をなす形で、研究計画の言語化訓練として価値あり
  - **配置案**:
    - 案A: 第10回（研究の新規性分析とポートフォリオ）の演習1の続きに、「DC1の研究目的・内容欄（2ページ）を書く」を20-25分追加
    - 案B: 第10回の演習2（GitHubポートフォリオ構築・25分）を選択制にし、博士進学希望者は「DC1ドラフト」、それ以外は「ポートフォリオ」
  - **第03回（ガクチカ）に入れない理由**: 既に演習50分で満杯、第2回フィードバック追加で導入も膨らみタイト。ガクチカ末尾の「発展課題」一行（学振申請書の業績欄にも適用）に留める
  - **親和性**: DC1の「研究目的・内容」「研究の独自性・創造性」欄は第10回の3観点（方法論的／実践的／時間的）の新規性分析と直結
  - **要検討**: DC1を出さない学生（半分以上の見込み）への位置づけ説明（「研究を言語化する訓練として価値がある」と明示）
