...
日付 & AIの誤り & 人間による修正 \\
\midrule
1/15 & 効果量$r$の計算で全標本数$n$を使用 & Rosenthal\cite{rosenthal1991}に基づき、$n_{\text{nonzero}}$（ゼロ差分を除いた標本数）を使用するよう修正 \\
...
\begin{screen}
\textbf{ヒント：} この事例は、著者の研究リポジトリのコミット履歴で全過程を追跡できる。読者も自分の研究でGitを使えば、AIとの協働のプロセスを同様に記録・検証できる。第\ref{chap8}章で解説するGitの活用は、AI時代の研究では単なるコード管理を超えた意味を持つ。
\end{screen}
