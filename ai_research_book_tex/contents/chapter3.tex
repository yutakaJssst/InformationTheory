...
\subsection{従来の調査プロセスとAI活用の比較}

AIを活用した文献調査は、従来の調査プロセスを根本的に変えるものではなく、各段階を加速・拡張するものである。表\ref{tab:traditional-vs-ai}に、従来の調査プロセスとAIを活用した調査プロセスの比較を示す。

\begin{table}[htbp]
\centering
\caption{従来の文献調査とAIを活用した文献調査の比較}
\label{tab:traditional-vs-ai}
\footnotesize
\begin{tabular}{lp{3.2cm}p{3.8cm}}
\toprule
項目 & 従来の調査プロセス & AIを活用した調査プロセス \\
\midrule
キーワード発見 & 自分の知識と経験に依存 & AIとの対話でアイデアを拡張 \\
全体像の把握 & 論文を1本ずつ読む & AIで分野の全体像を短時間で把握 \\
論文の深掘り & 原文を精読 & AIで構造的に要約し、重要部分を精読 \\
思考の整理 & 自分の頭の中で整理 & AIに壁打ちして思考を言語化 \\
プロセス & 直線的（検索→読む→まとめ） & 反復的・探索的サイクル \\
\bottomrule
\end{tabular}
\end{table}

\begin{itembox}[l]{注意：機密情報の取り扱い}
AIツールに未発表の研究データ、特許出願前のアイデア、個人情報などの機密情報を入力してはならない。入力されたデータがAIの学習に使用される可能性があるためである。特に、共同研究先との秘密保持契約（NDA）が存在する場合は、契約内容を確認した上でAIツールの利用範囲を判断すること。
\end{itembox}
...
\textbf{主なツール:} Gemini、ChatGPT（GPT-4o）、Claude
...
私は「[研究テーマ]」について文献調査を行いたいと\\
考えています。この研究テーマに関連する検索キーワードを、\\
以下の観点から網羅的にリストアップしてください。
...
私は「グラフニューラルネットワークによる薬物相互作用予測」\\
について文献調査を行いたいと考えています。\\
この研究テーマに関連する検索キーワードを、\\
以下の観点から網羅的にリストアップしてください。
...
What are the current state-of-the-art approaches for\\
...
\item \textbf{手法（Method）}: どのようなアプローチ・手法を\\
提案しているか？技術的な新規性は何か？
\item \textbf{結果（Results）}: 主要な実験結果は何か？\\
既存手法と比較してどの程度の改善が得られたか？
\item \textbf{限界（Limitations）}: この研究の限界や未解決の\\
課題は何か？今後の研究の方向性として何が示唆されているか？
\end{enumerate}
...
\item 私の研究の新規性を、学術論文のIntroductionに書くような\\
形式で3--4文にまとめてください。
\item この新規性の主張に対して、査読者が指摘しそうな\\
弱点は何か？
\end{enumerate}
\end{promptbox}

\paragraph{差分マトリクスの作成}\mbox{}

\begin{promptbox}
\textbf{【差分マトリクス作成プロンプト】}

以下の先行研究と私の研究について、\\
差分マトリクス\index{さぶんまとりくす@差分マトリクス}を\\
作成してください。
...
自分の研究の優位点には$\bigstar$をつけてください。
\end{promptbox}

\begin{screen}
\textbf{ヒント:} 新規性の言語化は、AIの出力をそのまま使うのではなく、あくまで「たたき台」として活用すること。最終的な新規性の主張は、指導教員や研究仲間と議論して磨き上げることが重要である。
\end{screen}
...
AIは非常に有用なツールであるが、その出力を無批判に信頼することは危険である。特に文献調査では、以下のような問題が発生する可能性がある。
...
Google Scholarで確認できるように、正確なタイトルと\\
著者名を記載してください。
\end{promptbox}
...
AIの回答には、学習データに起因するバイアス\index{ばいあす@バイアス}が含まれる可能性がある。文献調査で注意すべきバイアスには以下がある。
...
\vspace{5mm}
\begin{screen}
\textbf{本章のまとめ:} AIは文献調査を効率化する強力なツールであるが、万能ではない。AIの出力を鵜呑みにせず、常にファクトチェックを行い、批判的思考を維持することが必要である。文献調査の目的は、先行研究を知ることだけでなく、自分の研究の新規性と価値を明確にすることにある。AIを「思考の補助輪」として活用し、自分自身の研究力を高めてほしい。
\end{screen}
